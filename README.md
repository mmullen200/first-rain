...
Animals do not follow a global checklist. Each role responds independently to sustained local Habitat Support: a fixed Eusocial Colony needs fairly dry Detritus around its hive; a Flying Reproductive Vector needs separated flowering patches; grazers need concentrated forage beside living canopy cover; Wetland Engineers need flowing wetland, aquatic consumers, and building material together; and predators need a nearby grazer range. Colony settlement is deliberately legible before it completes: isolated scouts enter from the basin edge, repeated traffic disturbs one candidate patch, and only continuously suitable ground eventually receives an anthill. Residents can leave when local relationships collapse and return if the habitat recovers. Different cells can therefore support different roles at once, and the player needs a spatial Ecological Mosaic rather than one uniformly lush patch.
...
First Rain is in an early prototyping phase in Godot. The current playable work is a deliberately throwaway vertical slice used to test the opening survival rhythm, ecological observation and intervention, a connected Crash Basin, succession, autonomous animal roles, procedural weather, the Presence's communication, and the conditions leading to First Rain. The latest experiment is tracked in [issue #28](https://github.com/mmullen200/first-rain/issues/28) on branch `prototype/reversible-animal-residency`; it replaces the earlier animal checklist with sustained local arrival, departure, and return while retaining the parallel aquatic sulfur pathway.

Automated regressions establish that these systems behave consistently; they do not prove that the habitat changes, colony prospecting, spatial puzzle, or path to First Rain are understandable or enjoyable. Issue #28 therefore remains open pending its explicit playtest gate. Prototype quantities, timings, thresholds, visuals, and one-off fixtures are evidence for decisions rather than final production content.
