...
Organisms matter because of the roles they perform. Microbial Crust prepares damp loose ground; moss retains water and supplies Detritus; fungus decomposes dead matter and detoxifies ground; Rooted Mats build on that pioneer substrate but begin competing for water and nutrients; and Ground Flowering creates an offer that a Flying Reproductive Vector can carry between separated patches. That connection permits the much slower Canopy-Former to wake.

Animals do not follow a global checklist. Each role responds independently to sustained local Habitat Support: a fixed Eusocial Colony needs a living fungus garden around its hive; a Flying Reproductive Vector needs separated flowering patches; grazers need concentrated forage beside living canopy cover; Wetland Engineers need flowing wetland, aquatic consumers, and building material together; and predators need a nearby grazer range. Nothing in the world arrives from outside; every organism starts dormant. The Eusocial Colony sleeps as queens sealed into chambers at the foot of some hoodoos. A queen stirs when living fungus grows beside her hoodoo, opens her chamber if it holds, and founds the colony beside the hoodoo; if the fungus fails while she is waking, she dies. A colony whose garden fails seals itself back into its hoodoo and can wake again. Other animals still arrive from outside for now and are to be made dormant in turn. Residents can leave when local relationships collapse and return if the habitat recovers. Different cells can therefore support different roles at once, and the player needs a spatial Ecological Mosaic rather than one uniformly lush patch.

Weather is part of the same system. First Rain—the event for which the project is named—is the first sustained, surface-reaching rainfall to measurably change the basin. In the current experiment, standing-water producers create a sulfur precursor and aquatic consumers process it into volatile sulfur. That Aquatic Sulfur Pathway must join terrestrial vapor, biological particles, and a favorable natural weather window; a restored dry landscape alone is not enough. First Rain is neither a scripted reward nor an ability granted by the planet.
...
First Rain is in an early prototyping phase in Godot. The current playable work is a deliberately throwaway vertical slice used to test the opening survival rhythm, ecological observation and intervention, a connected Crash Basin, succession, autonomous animal roles, procedural weather, the Presence's communication, and the conditions leading to First Rain. All prototype work is merged to `main`; the latest experiment is the Four Bowls terrain ([issue #36](https://github.com/mmullen200/first-rain/issues/36)), a 48 × 32 Crash Basin with a mountain, separate water-holding lows and an eastern outlet. Project direction and open questions are tracked in [issue #1](https://github.com/mmullen200/first-rain/issues/1).

Automated regressions establish that these systems behave consistently; they do not prove that the habitat changes, queen waking, spatial puzzle, or path to First Rain are understandable or enjoyable. The earlier prototype issues were closed without recorded playtest results, so none of their playtest gates counts as passed; each new playtest finding is recorded in its own GitHub issue. Prototype quantities, timings, thresholds, visuals, and one-off fixtures are evidence for decisions rather than final production content.
