...
Throwaway Godot vertical slice for deciding whether the opening teaches movement, finite supplies, scientific observation, ecological intervention, recovery, cultivation, and the first sighting of the Presence through embodied play. The current branch asks whether a locally reversible ecological dependency sequence makes primitive life visibly build toward increasingly fragile animal roles while the standing-water sulfur pathway remains a distinct requirement for First Rain.
...
Can the player identify that microbial crust, moss turnover, fungal decomposition, rooted mats, and unanswered ground flowering prepare successive opportunities; that a colony precedes a flying reproductive vector, canopy, grazers, Wetland Engineer, and predator; and that aquatic producers plus consumers—not terrestrial greening alone—supply the volatile sulfur contribution required for First Rain?
...
Animal arrival uses local habitat shape instead of fixed destinations or one shared measure of lushness. The prototype scans bounded neighborhoods for species-specific contrasts: fairly dry Detritus for colonies, separated ground-flowering clusters for reproductive vectors, an edge between concentrated open forage and nearby cover for grazers, standing water and plant material along the Drainage Spine for Wetland Engineers, and living grazer proximity for predators. A uniform lush patch does not qualify every species. Ecological prerequisites now determine which role may search next: ground flowering precedes the colony; the colony precedes the vector; vector pollination wakes canopy; canopy precedes grazers; aquatic consumers and grazers precede the Wetland Engineer; and the Engineer precedes the predator. An animal is registered at the best qualifying Ecological Cell, so different habitat mosaics still produce different arrival locations.

This is still a provisional colonization slice. Animals appear directly at the selected destination, become permanent residents, and use fixed prototype population gates. Resident attraction or conflict, departure after habitat collapse, provisional scouts, visible entry from the basin rim, and resource-supported population size remain separate playable questions.
...
The checks verify the opening, movement, grazer behavior, scanner evidence, replay record, toolkit economy, basin traversal, survival pacing, disturbance recovery, shared animal authority, habitat-driven arrival, fixed-hive worker transport, the bounded arrival-search frame cost, and the ordered ecological prerequisites. Integrated checks additionally verify Detritus-dependent fungus, separate pioneer requirements for rooted mats, pollination-gated canopy, the producer/consumer sulfur pathway, reproduction without spontaneous biomass, distinct colony/vector/engineer effects, seeded weather that produces ecological disturbances plus ecosystem-enabled First Rain without a fixed schedule or Presence trigger, and an end-to-end route from finite watering and transplantation through every terrestrial and aquatic milestone.
