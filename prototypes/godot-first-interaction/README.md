...
Qualifying habitat must persist across repeated observations before settlement. Each resident remains associated with that local range; mobile foraging is bounded around it. If its support stays below the same habitat threshold through a grace period, the animal leaves for the Regional Ecology without dying. Recovery can support its later return. The fixed Eusocial Colony hive follows the same rule while present: the nest stays in one place and only its slow worker trail moves.

This remains a provisional colonization slice. Animals still appear directly at a selected destination and use a small fixed roster. Resident attraction or conflict, provisional scouts, visible entry from the basin rim, and fully resource-supported population size remain separate playable questions.
