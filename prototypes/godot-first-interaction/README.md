...
Animal arrival now uses local habitat evidence instead of basin-wide milestone counts and fixed destinations. The prototype scans bounded neighborhoods for species-specific conditions: damp Detritus for colonies, multiple nearby blossoms for reproductive vectors, standing water beside plant material for Wetland Engineers, concentrated forage for grazers, and living grazer proximity for predators. An animal is registered at the best qualifying Ecological Cell, so habitat created in a different part of the basin produces a different arrival location. Arrival discoveries describe tracks, movement, or settlement without announcing the species' ecological function; the evidence stream retains the provisional habitat score and measurements for observer analysis.

This is the first slice of the colonization question. Animals still appear directly at the destination, become permanent residents, and use one-per-species gates. Departure after habitat collapse, provisional scouts, visible entry from the basin rim, and resource-supported population size remain separate playable questions.
